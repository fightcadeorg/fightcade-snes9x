...
int _remote_behind_queue_graph[MAX_GRAPH_SIZE];
...
draw_network_graph_control(LPDRAWITEMSTRUCT di)
{
   ...
   if (true) {
      ...
      draw_graph(di, _green_pen, _local_behind_queue_graph,	_graph_size, -MAX_FAIRNESS, MAX_FAIRNESS);
      draw_graph(di, _yellow_pen,_remote_behind_queue_graph,	_graph_size, -MAX_FAIRNESS, MAX_FAIRNESS);
   }
}
...
ggpoutil_perfmon_update(GGPOSession *ggpo, GGPONetworkStats &stats)
{
   ...
   if (_graph_size < MAX_GRAPH_SIZE) {
	 ...
	 * Averages
	 */
	 //_local_behind_queue_graph[i] = (int)local_behind;
	 //_remote_behind_queue_graph[i] = (int)remote_behind;
