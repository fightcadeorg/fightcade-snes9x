...
void CheckMenuStates();
void ResetFrameTimer();
bool LoadROM(const TCHAR *filename, const TCHAR *filename2 = NULL);
bool LoadROMMulti(const TCHAR *filename, const TCHAR *filename2);
...
void S9xRestoreWindowTitle()
{
	...
	if (Memory.ROMFilename[0])
	{
		...
		_stprintf(buf, TEXT("%s - %s %s-%s"), (wchar_t *)Utf8ToWide(def), WINDOW_TITLE, TEXT(VERSION), TEXT(FCVERSION));
	}
	else
		_stprintf(buf, TEXT("%s %s-%s"), WINDOW_TITLE, TEXT(VERSION), TEXT(FCVERSION));
	...
	LPARAM lParam)
{
	...
	TCHAR hostname[100];
#endif
	if (uMsg > 1000 && uMsg != 0x432 && uMsg != 0x433)
	{
		i = 0;
	}
	switch (uMsg)
...
bool ProcessFrameDelay(int frames)
{
	int ms = (int)(frames / 60);
	if (ms < 0) {
	...
	int nCmdShow)
{
	...
	if (rom_filename)
	{
		if (_tcsncmp(rom_filename, _T("quark:"), 6) == 0)
			QuarkInit(rom_filename);
		else
		{
			...
			{
				wsprintf(multiRomA, L"%s.zip", rom_filename);
				LoadROM(multiRomA);
			}
...
bool LoadROMPlain(const TCHAR *filename)
{
